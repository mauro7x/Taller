...
bool LTexture::loadFromFile(std::string path) {
    ...
    if (loadedSurface == NULL) {
        ...
        SDL_Surface* formattedSurface = SDL_ConvertSurfaceFormat(
            loadedSurface, SDL_GetWindowPixelFormat(gWindow), 0);
        if (formattedSurface == NULL) {
            printf(
                "Unable to convert loaded surface to display format! SDL "
                "Error: %s\n",
                SDL_GetError());
        } else {
...
bool loadMedia() {
    ...
    if (!gFooTexture.loadFromFile("foo.png")) {
        ...
        // Manual color key
            ...
            int pixelCount =
                (gFooTexture.getPitch() / 4) * gFooTexture.getHeight();

            // Map colors
            Uint32 colorKey = SDL_MapRGB(mappingFormat, 0, 0xFF, 0xFF);
            Uint32 transparent =
                SDL_MapRGBA(mappingFormat, 0xFF, 0xFF, 0xFF, 0x00);
